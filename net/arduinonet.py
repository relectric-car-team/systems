...
class ArduinoNet():
	""" Initializes an ArduinoNet object, given the serial device the connection
		is to be made with. The returned object will already be connected and
		ready to send or receive data, provided there were no problems
	...
	""" Sends a value to the Arduino. The value's meaning can be specified by
		key, and integer that is associated with a particular purpose, as 
