...
""" The Systems class is the main object that encapsulates the program for the
	...
	this class owns an instance of NetworkManager, an object containing
	instances of all the necessary abstracted networking interfaces needed to
	communicate	with the aforementioned sub-systems. The NetworkManager is
	passed to each controller upon initialization.
"""
class Systems():
	...
	""" Finds and returns the value of the specified variable that belongs to
	...
	""" Finds and sets the value of the specified variable that belongs to one
		of the controllers.
		...
		while True:
			...
			if request != None:
				if request["type"] == "action":
					response = sendAction(request["name"], 
						tuple(request["args"]))
